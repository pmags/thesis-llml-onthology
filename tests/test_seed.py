...
class TestCreateSeedOntology:
    ...
    def test_create_seed_ontology_graph_structure(self, ontology_with_seed):
        ...
        assert len(graph.edges()) == 6, f"Expected 6 edges, got {len(graph.edges())}"
    ...
    def test_create_seed_ontology_node_attributes(self, ontology_with_seed):
        ...
        graph = ontology_with_seed.ontology_graph
        required_attrs = {"term", "description", "level", "n_visits", "total_reward"}
        ...
        for node_id in graph.nodes():
            ...
            assert graph.nodes[node_id]["level"] in {"class", "subclass", "instance"}
            assert isinstance(graph.nodes[node_id]["n_visits"], int)
            assert isinstance(graph.nodes[node_id]["total_reward"], (int, float))
    ...
    def test_create_seed_ontology_edge_attributes(self, ontology_with_seed):
        ...
        for source, target in graph.edges():
            ...
            assert relation in {"subClassOf", "type"}, f"Invalid relation: {relation}"
    ...
    def test_create_seed_ontology_duplicate_handling(self, ontology_empty):
                    ...
                    "subclasses": [
                        {
                            ...
                            "instances": [
                                ...
                                {"term": "Instance1", "description": "Inst 1 duplicate"}  # Duplicate
                            ]
                        }
                    ]
                }
        ...
        assert len(graph.nodes()) == 3, f"Expected 3 unique nodes, got {len(graph.nodes())}"
